...
importMiniui(function () {
    ...
    require(["miniui-tools"], function (tools) {
        ...
        $(".add-button").click(function () {
            tools.openWindow("admin/education/video/save.html", "添加视频", "800px", "80%", function (e) {
                grid.reload();
            })
        });
        search();
...
function edit(id) {
    tools.openWindow("admin/education/video/save.html?id=" + id, "编辑视频", "800px", "80%", function (e) {
        grid.reload();
    })
}
        ...
        tools.createActionButton("删除", "icon-remove", function () {
            if (row._state == "added") {
                ...
                require(["request", "message"], function (request, message) {
                    message.confirm("确定删除该视频?", function () {
                        var loading = message.loading("删除中...");
                        request["delete"]("video-master/" + row.id, {}, function (res) {
                            loading.close();
                            if (res.status == 200) {
                                e.sender.removeNode(row);
                            } else {
